...
extension __ {
    ...
    class func partial<T, U, V>(f: (T, U) -> V, arg: T ) -> ( U -> V ) {
        return { f(arg, $0) }
    }
    
    // partially act to function which takes 3 variables
    class func partial<T, U, V, W>(f: (T, U, V) -> W, arg: T ) -> ( (U , V) -> W ) {
        return { f(arg, $0, $1) }
    }
    
    // partially act to function which takes 4 variables
    class func partial<T, U, V, W, X>(f: (T, U, V, W) -> X, arg: T ) -> ( (U , V, W) -> X ) {
        return { f(arg, $0, $1, $2) }
    }
    
    // partially act to function which takes 5 variables
    class func partial<T, U, V, W, X, Y>(f: (T, U, V, W, X) -> Y, arg: T ) -> ( (U , V, W, X) -> Y ) {
        return { f(arg, $0, $1, $2, $3) }
    }
    
    // partially act to function which takes 6 variables
    class func partial<T, U, V, W, X, Y, Z>(f: (T, U, V, W, X, Y) -> Z, arg: T ) -> ( (U , V, W, X, Y) -> Z ) {
        return { f(arg, $0, $1, $2, $3, $4) }
    }
    
    // partially act to function which takes 7 variables
    class func partial<T, U, V, W, X, Y, Z, A>(f: (T, U, V, W, X, Y, Z) -> A, arg: T ) -> ( (U , V, W, X, Y, Z) -> A ) {
